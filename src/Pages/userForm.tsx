  ...
  const userSchema = Yup.object().shape({
    ...
    email: Yup.string()
      .test('includes-char', 'โปรดใส่ @', (value) => {
        return value ? value.includes('@') : false; // ตรวจสอบว่ามีตัว @
      })
      .test('includes-dot', 'โปรดใส่ . (จุด)', (value) => {
        return value ? value.includes('.') : false; // ตรวจสอบว่ามีตัว . 
      })
      .test('no-spaces', 'อีเมลต้องไม่มีช่องว่าง', (value) => {
        return value ? !value.includes(' ') : true; // ตรวจสอบว่าไม่มีช่องว่าง
      })
      .email('รูปแบบอีเมลไม่ถูกต้อง เช่น user@example.com') // ตรวจสอบรูปแบบอีเมล
      .required('กรุณากรอกอีเมล'), // ตรวจสอบว่าฟิลด์ไม่ว่าง
  });
